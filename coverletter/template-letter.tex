...
\parbox[t]{.49\linewidth}{\raggedright \includegraphics$if(signature-width)$[width=$signature-width$]$endif${$signature$}}
$endfor$
$if(author)$\\
$if(signature-after)$\vspace*{$signature-after$}$endif$
$for(author)$\parbox[t]{.49\linewidth}{\raggedright%
$if(author.name)$$author.name$\\ \medskip $endif$
$if(author.job)$$author.job$\\ \smallskip $endif$
$if(author.affiliation)$ $for(author.affiliation)$ $author.affiliation$\\ $endfor$ \smallskip $endif$
...
$if(letterfoot)$
\usepackage{wallpaper}
\LLCornerWallPaper{1}{$letterfoot$}
$endif$
...
$endif$
% temporarily overwrite \indentedwidth to ensure we can print across the entire width
\newcommand{\tempwidth}{\indentedwidth}
\indentedwidth=\textwidth 
\closing{$closing$}
\indentedwidth=\tempwidth
$endif$
$if(encl)$
