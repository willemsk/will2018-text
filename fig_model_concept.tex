...
(\subref{fig:clya_side}) Top and (\subref{fig:clya_top}) cross-sectional views of the dodecameric nanopore 
...
(\subref{fig:model_geometry_zoom}+\subref{fig:model_geometry}) The 2D-axisymmetric simulation geometry of 
ClyA (grey) embedded in a lipid bilayer (green) and surrounded by a spherical water reservoir (blue).
Note that all electrolyte parameters depend on the local average ion concentration 
$\avionconc=\frac{1}{n}\sum_{i}^{n}\concentration_{i}$ and that some are also influenced by the distance 
from the nanopore wall $\walldistance$.
