...
(\subref{fig:clya_side}) Cross-sectional and (\subref{fig:clya_top}) top views of an all-atom model of the 
...
($\potential = \vbias$), red), while the \cis\ side is kept grounded ($\potential 
= 0$, blue). The fixed concentration ($\concentration_i = \concentration_{\text{s},i}$) and no normal 
