...
the monitoring of blood glucose levels\cite{Chen-2013} to \todo{??}.
...
build solid state nanopores\todo{ref} since both shape and size affect the resulting signal ion currents 
...
model with dependencies on both the average ion concentration %$\ca$
...
these respective voltages.\todo{ref} Another possibility could also be that our mobility model, which makes 
...
($\conductance(\cbulk)=a\cbulk^{\gamma}$) yielded exponents $\gamma$ of $0.498\pm0.004$~$\text{M}^{-1}$ and 
$1.000\pm0.016$~$\text{M}^{-1}$, respectively. while the linear dependency on $\cbulk$ is commonly 
observed at high salt concentrations\todo{ref}, the square-root dependency at low salt suggests a transport 
...
fitting yielded $\gamma$ value of $0.614\pm0.009$~$\text{M}^{-1}$ and $0.787\pm0.001$~$\text{M}^{-1}$ for 
...
\subsection{Electrostatic potential and energy}\label{sect:electrostatic_potential}
...
pore.\cite{Bhattacharya-2011}\todo{more ref}. In the following section we aim to describe the most salient 
features of the potential (\cref{fig:potential}) and to link it back to the observed ionic conductance 
properties through the average electrostatic energy of cations and anions (\cref{fig:potential_energy}).

\paragraph{Influence of the fixed charges.}
To demonstrate the extend to which ClyA's fixed charges ($\scdpore$) influence the distribution of the 
electrostatic potential inside the pore, we plotted the contourmaps of the potential for representative 
concentrations over the entire investigated concentration range (\cref{fig:potential_contours}).
At low salt concentrations ($\cbulk<0.15$~M), the lack of ionic screening results in a highly negative 
potential in the entire pore. At medium concentrations ($0.15<\cbulk<1$~M), the center of the pore becomes 
almost neutral, with well-defined, highly negative `hotspots' whose influence extends significantly into the 
`bulk' of the pore. They are located near the \cis\ entry of the pore ($10<z<12$~nm), in the middle of the 
lumen ($4<z<6$~nm) and inside the \trans\ constriction ($1.85<z<1.60$~nm). At very high concentrations, the 
To be able to better quantitatively describe the modulation of the electrostatic potential, we computed its 
average radial value along the nanopore's length
\begin{align}
...
constriction ($\Delta E_{\text{B},i}$, \cref{fig:potential_energy_trans_barrier}).
...
for entry in the lumen at $z\approx1.6$~nm (\cref{fig:potential_energy_trans_barrier}, blue curves). At very 
low ionic strengths ($\cbulk<0.025$~M), the energy at \trans\ is significantly lower than that the final 
energy in the \cis\ compartment (e.g. $\Delta\radenergy>2$~$\kT$ at $0.005$~M), forcing the ions to 
accumulate 
inside the pore. At higher concentrations ($\cbulk>0.05$~M), the increased screening smooths out the 
...
they face the second, more significant energy barrier (\cref{fig:potential_energy_trans_barrier}, blue 
curves). This barrier reduces the number of anions that can exit the pore, causing them to accumulate inside 
the pore and increasing their concentration. Ions that pass the final barrier are rewarded with a large 
reduction of their electrostatic energy, as the bulk of the voltage drop occurs only \emph{after} the \trans\ 
constriction.

\paragraph{Energy landscape at $\mathbf{-150}$~mV.}
For cations, translocation at negative voltages occurs from \cis\ to \trans\, with a gradual drop of the 
electrostatic energy throughout the lumen of the pore up until the \trans\ constriction
(\cref{fig:potential_energy_radial_averages}, top right). This promotes the efficient removal of cations 
...
barrier (\cref{fig:potential_energy_trans_barrier}, red curves), which reduces the nanopore's ability to 
conduct cations compared to positive potentials and hence contributes to the ion current rectification.
 
The situation for anions at negative bias voltages, where they must go from \trans\ to \cis\, is very 
different (\cref{fig:potential_energy_radial_averages}, bottom left), as they are faced with overcoming the 
...
The constrictions present in many biological nanopores often play a crucial role in shaping the ionic 
conductance properties as the confinement of charged residues results in large electrostatic energy 
barriers.\cite{Maglia-2008,Franceschini-2016,Huang-2017} With its highly negatively charged \trans\ 
constriction, ClyA's affinity for the transport of anions is significantly diminished, resulting in high 
...
%pore's ionic transport properties. At negative voltages, $\deltaEt$ 
