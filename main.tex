...
landscape within ClyA for both cations and anions. We will conclude this section by discussing the properties
...
\paragraph{Ionic current.} The ionic current-voltage (IV) relationship of a ClyA---and that of many nanopores
ingeneral--- often deviates significantly from Ohm's law. This is because the ionic flux results from a
complex interplay between the pore's geometry (size, shape and charge distribution), the properties of the
surrounding electrolyte (salt concentration/type, viscosity, ...) and any externally applied conditions (bias
voltage, temperature, pressure, ...). Hence, if a computational model is capable of quantitatively predicting
the ionic current of a nanopore over a wide range of bias voltages and salt concentrations, this is a strongly
indicates that it captures the essential physics governing the nanopore.
...
charge distributions, over the entire experimentally accessible concentration and voltage ranges.
...
\num{-200} to \SI[retain-explicit-plus]{+200}{\mV} (\cref{fig:conductance_contourmap_epnp}). The high degree of electrostatic screening at high salt concentrations ($>\SI{1}{\Molar}$) results in a conductance that is virtually independent of the applied bias voltage. t lower ionic strengths, the conductance begins to lower as the nanopore's charges become sufficiently shielded as not to influence conductance is independent of bias voltage
