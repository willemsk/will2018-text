...
coined extended-PNP-NS (ePNP-NS), which improves their accuracy and validity for nanoscale model through the
...
range of ionic strengths and applied potentials. We found that, due to ClyA's unique shape and charge 
distribution, the electro-osmotic flow does not decrease monotonically with salt concentration but rather 
exhibits a maximum at $\approx500$~mM.

We further report on the influence of both the bias voltage 
and the bulk ionic 
strength on the average ion concentrations inside the pore, the magnitude and direction of the 
electro-osmotic flow, and the shape of the electrostatic potential landscape inside the nanopore.
\end{abstract}
\newpage
% Introduction
...
($\conductance(\concentration_\text{bulk})=a \concentration_\text{bulk}^{b}$) yielded exponents of 
...
other. In the case of a binary electrolyte such as \ce{NaCl}, cation selectivity indicates that the 
contribution of \ce{Na+} to the overall current is larger than that of \ce{Cl-}, and vice versa for anion 
selectivity. Here, we represent the ion selectivity by the \ce{Na+} transport number
\begin{align*}
\transportn_{\ce{Na+}} =
...
(\cref{fig:transport_number_contourmap_epnp}). ClyA is cation selective ($\tna > 0.5$) for all investigated 
voltages up to a bulk concentration of $\approx2$~M, after which it drops below $0.5$, resulting in a anion 
selective pore. Note that the $\tna$ at $5$~M ($0.45$) still exceeds the value for bulk at that concentration 
($0.352$).
...
% fig:concentration_contours
% fig:concentration_radial_profiles
% fig:concentration_pore_average_vs_concentration
%
