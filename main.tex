...
fluid`s he viscosity. Hence, as also observed in MD simulations for water molecules near
...
concentrations ($<10$~mM), significantly overestimates the ionic mobility at at higher salt
...
imbalance. Next we describe ClyA's electrostatic potential landscape and link the magnitude of its
electrostatic barriers to the ionic current rectification and ion selectivity. We conclude with a
characterization of the concentration and voltage dependency of the electro-osmotic flow.

\input{fig_model_concept.tex}

\section{Mathematical model}\label{sect:math_model}
...
dynamical behavior of our complete system is described by the coupled Poisson, Nernst-Planck and Navier-Stokes
...
To improve quantitative accuracy of the PNP-NS equations for nanopores, we developed an \emph{extended}
version of these equations (ePNP-NS) using the commmerical finite element solver COMSOL Multiphysics
(www.comsol.com) that self-consistently takes into account 1) the finite size of the
...
relative permittivity at infinite dilution and $\relperm^c(\avionconc)$ an concentration dependent empirical
...
fitted to experimental data of the respectively the ion self-diffusion coefficient and the electrophoretic
...
and the nanopore radii. We chose not include this correction into our model, as extrapolating its
applicability for ions with hydrodynamic radii than the solvent is questionable.\cite{Anderson-1972,
Deen-1987}
...
viscosity $\viscosity$ and pressure $\pressure$. The external force $\vec{\force}$ that acts on the fluid is
given by
\begin{align}
...
All concentration dependent parameters use the average local ion concentration rather than their individual
concentrations. While valid for \emph{electroneutral} bulk solutions, this approximation no longer holds
inside the electrical double layer, i.e. near charged surfaces or inside small nanopores, where local
electroneutrality is violated. The main reasons for making this simplification are the lack of non-bulk
experimental data and the absence of a tractable analytical model. Nevertheless, we will see that the current
...
i.e. $\vec{\beta}=0$ and by setting all concentration and wall distance functions to unity, such that
...
full-atom homology model of ClyA-AS\cite{Soskine-2013} and extracted 50 sets of atomic coordinates of a from
an \emph{equilibrium} explicit solvent MD trajectory. For each of these structures, we computed a 2D atomic
density\cite{Li-2013} and charge\cite{Aksimentiev-2005} map (crf. methods for details). These maps were
averaged to obtain a version that captures the movement of the side chains. The nanopore geometry was then
defined as the \SI{25}{\percent} density contourline, which closely follows the outline of a \SI{30}{\degree}
wedge of the full atom structure (\cref{fig:model_geometry_vs_wedge}). The equilibrium charge map
(\cref{fig:model_charge_density}) was loaded directly into our solver as an interpolated function ($\scdpore$)
and applied to the entire computational domain.
...
parameters) that are impermeable impermeable to ions and water.
...
A Neumann no-flux condition ($-\vec{n}\cdot\vec{\flux}_{i} = 0$) and a no-slip ($\vec{\velocity} = 0$)
boundary condition were used to prevent the flux of ions and movement of fluid through the nanopore and the
bilayer computational domains. Finally, a Neumann boundary condition was applied at the bilayer external
boundary ($-\vec{n}\cdot\left(\absperm \relperm \nabla \potential \right) = 0$).


\section{Results and discussion}\label{sect:results}

The flow of water and ions through the ClyA nanopore is determined by both the geometrical features the pore
and the physical properties of the electrolyte that surrounds it. To realistically describe the ionic
currents, ion concentration distributions, electrostatic fields and water fluxes, a nanopore simulation should
thus contain an accurate representation of both the pore and the electrolyte. Due to their explicit atom
nature, molecular dynamics (MD) simulations intrinsically capture most, if not all, relevant transport
processes occurring in nanoscale pores. However, a full atom MD simulation of ClyA (\num{\approx 700000}
atoms) requires the use of a supercomputer to obtain results in a reasonable amount of time, as each change in
voltage or salt concentrations requires multiple runs of \SI{100}{\ns} or more.\cite{Aksimentiev-2004,
Bhattacharya-2011}

In this work, we opted for a much more computationally efficient approach, in which both the nanopore and the
electrolyte are modeled as a continuum rather than discrete atoms, and the electrostatics, ion transport and
water flow are described by the coupled Poisson-Nernst-Planck and Navier-Stokes (PNP-NS)
equations.\cite{Simakov-2010,Pederson-2015,Aguilella-Arzo-2017,Simakov-2018} To further increase the
computional speed we reduced ClyA's complex 3D geometry to a 2D-axisymmetric model --- a reasonable
approximation considering that it consists of many identical subunits, arranged in a radially symmetric manner
around the nanopore axis (\cref{fig:clya_top}).

The continuum approach makes use of spatially uniform material properties that emerge from the average
behaviour of small groups of atoms, i.e. relative permittivity, diffusion coefficient, electrophoretic
mobility, viscosity and density. However, because these properties are often influenced strongly by the
\emph{local} intramolecular interactions, they should not be treated as a spatially uniform constants, but
rather as self-consistent parameters that depend on the \emph{local} conditions inside the system. For a
nanopore at constant temperature (\SI{298.15}{\kelvin}), we identified the salt concentration
\cite{Baldessari-2008-1,Burger-2012,ContrerasAburto-2013-1} and the distance from the pore
wall\cite{Makarov-1998,Simakov-2010,Pronk-2014} as being the most influential factors, and to this end we
created an extended-PNP-NS (ePNP-NS) framework in which the material properties depend self-consistently on
the on both the average ion concentration $\avionconc$ and distance from the pore wall $\walldistance$.

To validate our model, we experimentally measured the single channel ionic conductance of ClyA at a wide range
of salt concentrations and bias voltages, and compared them with the simulated ionic transport properties, in
terms of current, conductance, rectification and ion selectivity, of both the classical PNP-NS and the newly
developed ePNP-NS equations. After validation, we proceed with describing the influence of bias voltage and bulk ionic strengths on the local ion concentrations inside the pore and the resulting electrical double layer. This is followed by a characterization of the electrostatic potential and the electrostatic energy landscape within ClyA for both cations and anions. We conclude by discussing the properties of the electro-osmotic flow.
...
To further investigate ClyA's ionic transport mechanism, we computed the ionic conductance
...
molecular density. Briefly, 50 sets of atomic coordinates were extracted from the final 5~ns of the
...
of all 50 structures on a 0.5~\AA\ resolution grid using the Gaussian function\cite{Li-2013}
