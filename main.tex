...
Typhii} artificially evolved for improved stability\cite{Soskine-2013}---and extracted 50 sets of atomic
coordinates of a from an \emph{equilibrium} explicit solvent MD trajectory. For each of these structures, we
computed a 2D atomic density\cite{Li-2013} and charge\cite{Aksimentiev-2005} map (crf. methods for details).
These maps were averaged to capture the various conformations and motions of the side chains. The nanopore
geometry was then defined as the \SI{25}{\percent} density contourline, which closely follows the outline of a
...
developed ePNP-NS equations. After validation, we proceed with describing the influence of bias voltage and
bulk ionic strengths on the local ion concentrations inside the pore and the resulting electrical double
layer. This is followed by a characterization of the electrostatic potential and the electrostatic energy
landscape within ClyA for both cations and anions. We conclude by discussing the properties of the
...
\paragraph{Ionic current and conductance.} The ionic conductance of a nanopore---a quantity that can be
measured experimentally with relative ease---is determined not only by the its geometry and charge
distribution, but also depends non-trivially on the magnitude of the bias voltage and the ionic strength.
Hence, the nanopore conductance \emph{landscape}, i.e. it value as a function of bulk ionic strength and
applied bias voltage, can serve as a powerful benchmark to gauge the ability of our model to reproduce the

To verify whether our model provides quantitative results for any condition, we performed steady-state simulations of ClyA over a wide range of conditions and compared them directly with a set of experimentally determined values (\cref{fig:current-voltage_curves}). ($\cbulk=\text{\num{0.005} to \SI{5}{\Molar}}$ \ce{NaCl} and $\vbias=\text{\num{-500} to \SI[retain-explicit-plus]{+500}{\mV}}$) using both the classical PNP-NS and our new ePNP-NS equations  ($\cbulk=\text{0.05, 0.15, 0.5, 1 and \SI{3}{\Molar}}$ \ce{NaCl} and \SIrange[retain-explicit-plus]{-150}{+150}{\mV}  . The ionic current was computed by integration and summation of the total ionic flux over the \cis\ reservoir boundary
\begin{equation}
...
normal vector.
 ...
 We c. Over the entire investigated
...
their `infinite dilution' values (\cref{suppinfo:fig:corrections}). Interestingly, while the ePNP-NS equations
show virtually no error at high ionic strengths (\SI{\ge0.5}{\Molar}), a minor overestimation () of the ionic
current is oblarger for lower salt concentrations, with smaller and larger currents at negative and positive
bias voltages, respectively. Their cause is likely to be found in the under- and overestimation of the cation
concentration at these respective voltages.\todo{ref} It is also possible that the mobility we use, which is
parameterized using electroneutral `bulk' values for unconfined ions, begins to break down under these
