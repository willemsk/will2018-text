...
%as they are able to provide direct and detailed information about the
%Computational methods have already contributed significantly to our qualitative understanding of the
%transport mechanisms of water and ions through confined, nanoscale geometries.
...
A full atom model of ClyA-AS\cite{Soskine-2013} was build and optimized (MODELLER v9.18) by introduction of the following point mutations in each of the 12 chains of the wild-type ClyA crystal structure (PDBID: 2WCD\cite{Mueller-2009}):
...
The conformation of all mutated side chains was then optimized with an annealing protocol (heating: 150, 250, 400, 700 and 1000~K, cooling: 1000, 800, 600, 500, 400 and 300~K). At each temperature, the energy was minimized for 200 iterations with a conjugate gradients algorithm (4~fs timestep).\cite{Shanno-1980} The anneal was performed twice, once without taking interactions with non-mutated residues into account and once with all non-bonded interactions.\cite{Sali-1993} The refined structure was then embedded in the center of an $18\times18$~nm$^2$ lipid bilayer patch (DPhPC lipid, created using the CHARMM-GUI membrane builder\cite{Lee-2016} and equilibrated using NAMD\cite{Philips-2005}) and subsequently hydrated by 214640 TIP3 water molecules, confined in a box of $18\times18\times32\text{nm}^3$. The global charge of the system was neutralized, and its salt concentration set to $\approx$0.15~M, by replacing 1276 random TIP3 molecules with 674 \Na\ and 602 \Cl\ ions (VMD v1.9.4\cite{Humphrey-1996}).\cite{Aksimentiev-2005}
