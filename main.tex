...
placing the nanopore between two electrolyte compartments and applying a constant voltage across them. Due to
the high resistance of the nanopore, virtually the full potential change occurs near and within the pore,
resulting in a strong electrical field (\SIrange{1e7}{1e8}{\mV\per\nm}) that electrophoretically drives ions
and water molecules through the pore.\cite{Wong-2007,Mao-2014,Haywood-2014,Laohakunakorn-2015} Hence, analyte
molecules such as DNA or proteins are driven towards and often \emph{through} the nanopore by a combination of
Coulombic (electrophoretic) and hydrodynamic (electro-osmotic) forces.\cite{Wong-2007,Grosberg-2010,
Muthukumar-2010,Muthukumar-2014} If successful such a translocation is observed as a temporal fluctuation in
the ionic conductance of the pore and serves as a unique molecular fingerprint with which the molecule can be
...
As observed in MD simulations, the interaction of ions or small molecules like water with the heavy atoms of proteins or DNA results in a strong reduction of their motility.\cite{Makarov-1998,Pronk-2014} Since these
effects occur at distances \SI{\le1}{\nm}, they can usually be neglected for macroscopic simulations.
However, in small nanopores ($\le10$~nm diameter), they comprise a significant fraction of the total nanopore
volume and hence must be taken into account.\cite{Noskov-2004,Simakov-2010,Pederson-2015,McMullen-2017} In
...
version of these equations (ePNP-NS) which self-consistently takes into account 1) the finite size of the
...
and \cref{suppinfo:tab:corrections_parameters}, respectively. The ePNP-NS equations were numerically solved with finite element method as implemented by the COMSOL Multiphysics software package (www.comsol.com).
...
$\viscosity$ and a concentration dependeny for the $\density$ by replacing their constant values with the
...
All concentration dependent functions (\cref{tab:corrections_equations}) were parametrized by fitting of a
polynomial function to experimental data obtained through bulk measurements
(\cref{suppinfo:tab:corrections_parameters} and \cref{suppinfo:fig:corrections}). Hence, out use of the
average local ion concentration rather than their individual concentrations is certainly valid for
\emph{electroneutral} bulk solutions, its applicability for regions where local electroneutrality is violated,
i.e. inside (overlapping) electrical double layers, is questionable. The main reasons for making this
simplification are the lack of non-bulk experimental data and the absence of a tractable analytical model.
Nevertheless, we will see that the current concentration dependent functions will lead to an excellent
