...
biological nanopores -- if the appropriate corrections are applied -- can make both qualitatively and
...
version of these equations (ePNP-NS) that self-consistently takes into account 1) the finite size of the
...
relative permittivity.\cite{Mills-1989,Hai-Lang-1996,Gavish-2016}
...
local relative permittivity $\relperm$. The charge densities $\scdpore$ and $\scdion$ represent the nanopore's
\emph{fixed} and the electrolyte's \emph{ionic} charge densities, respectively. In the fluid the relative
permittity is given by
\begin{align}
  \relperm(\avionconc)=\relperm^0 \relperm^c(\avionconc) \text{\,,}
\end{align}
with $\avionconc=\frac{1}{n}\sum_{i}^{n}\concentration_{i}$the average ion concencentration, $\relperm^0$ the
relative permittity at infinite dilution and $\relperm^c$ an empirical equation fitted to experimental data of
the relative permittivity at various \ce{NaCl} concentrations (cfr. methods).\cite{Gavish-2016}

\paragraph{Ionic flux.}
The the ionic flux $\flux_{i}$ of each ion $i$ at steady state is given by the size-modified Nernst-Planck equation\cite{Lu-2011}
\begin{align}
  \label{eq:sm-nernst-planck}
  \displaystyle\frac{\delta \concentration_{i}}{\delta t} ={}& - \nabla \cdot \flux_{i} = 0 \\
  \flux_{i} ={}&
...
$\velocity$, Avogadro's constant $\avogadro = \SI{6.022e23}{\per\mole}$ and \emph{steric} ion and water
diameters $\ionsize_{i}$ and $\ionsize_{0}$. Because currently there are no experimentally verified values
available for $\ionsize_{i}$ and $\ionsize_{0}$, we set them to \SI{0.5}{\nm} and \SI{0.311}{\nm}, such that
their limiting concentrations correspond to \SI{13.3}{\Molar} and \SI{55.2}{\Molar},
respectively.\cite{Bazant-2009}

\paragraph{Fluid flow.}
The fluid flow and pressure are given by the Navier-Stokes equations for incompressible fluids\cite{Axelsson-2015}
\begin{align}
  ...
  \text{\,,}
\end{align}
with fluid velocity $\velocity$, density $\density$, hydrodynamic stress tensor $\hydrostresstensor$, viscosity $\viscosity$, pressure $\pressure$ and body force $\vec{\force}$.

Note that because density and the viscosity of the fluid fluid density is not a constant, it's spatial derivative must be


%The $\avionconc=\frac{1}{n}\sum_{i}^{n}\concentration_{i}$
% =\relperm^0 \relperm^c
% =\diffusion_{i}^0\diffusion_{i}^c\diffusion_{i}^w
% =\mobility_{i}^0\mobility_{i}^c\mobility_{i}^w

% with $\relperm^0$ the value at infinite dilution and $\relperm^c$ an empirical function that depends on the
% average ion concentration (see methods for detailed expressions).\cite{Gavish-2016}
...
% The values of $\diffusion_{i}$, $\mobility_{i}$ and $\beta_{i}$ are
% \begin{align}
%   \diffusion_{i} =
%   \begin{cases}
...
\subsection{A 2D-axisymmetric model}
...
For \ce{NaCl} the parameters are given in \cref{tab:pnpns_parameters}. The driving force that generates the electro-osmotic flow
\begin{align}
